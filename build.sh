...
docker tag timwr/metasploit-framework $DOCKER_TAG
...
ssh -i ~/.ssh/google_compute_engine user@$HOST "sudo docker pull $DOCKER_TAG && sudo docker stop metasploit && sudo docker rm metasploit"
